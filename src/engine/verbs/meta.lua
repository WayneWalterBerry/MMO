...
function M.register(handlers)
    ...
    handlers["help"] = function(ctx, noun)
        -- Check if this is an E-rated world (kid-friendly)
        local is_e_rated = ctx and ctx.world and ctx.world.rating == "E"
        ...
        print("  feel                     Feel around in the dark (works without light)")
        ...
        -- Only show Tools & Crafting section if not E-rated, or filter it
        if not is_e_rated then
            ...
            print("  break <thing>            Break something breakable")
            print("  tear <thing>             Tear fabric apart")
            print("  light match on <x>       Light a match on a rough surface")
            print("  light <thing>            Light a candle or torch (needs fire)")
            print("  extinguish <thing>       Put out a flame")
            print("  write <text> on <thing>  Write on a writable surface")
            print("  sew <thing> with <tool>  Sew materials together (requires skill)")
        ...
        -- Combat section: only show in non-E-rated worlds
        if not is_e_rated then
            print("== Combat ==")
        ...
        -- Health & Survival section: filter for E-rated
        if not is_e_rated then
