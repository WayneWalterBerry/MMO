...
if handlers and handlers["go"] then
    ...
    test("2.05 'go north' with no portal falls back to legacy exit", function()
        -- Room has a legacy exit (inline table, no portal field)
        ...
        -- Remove portal objects from registry (pure legacy)
        ctx.registry:remove("bedroom-hallway-door-north")
        ctx.registry:remove("bedroom-hallway-door-south")
        capture_output(function() handlers["go"](ctx, "north") end)
        eq("hallway", ctx.player.location,
            "legacy exit should still work when no portal is present")
    end)
    ...
    pending("2.05 legacy exit fallback", "verb handlers not loaded")
...
---------------------------------------------------------------------------
suite("COMPAT: legacy exits and mixed rooms")

if handlers and handlers["go"] then
    test("6.01 room with only legacy exits still works", function()
        -- No portal objects — pure legacy exit table
        local reg = registry_mod.new()
        local room = {
            id = "legacy-room",
            name = "Legacy Room",
            description = "A room with old-style exits.",
            contents = {},
            exits = {
                east = {
                    target = "other-room",
        ...
        local ctx = {
            registry = reg,
            current_room = room,
            rooms = { ["legacy-room"] = room, ["other-room"] = other },
            player = {
        ...
        eq("other-room", ctx.player.location,
            "legacy exit movement should still work")
    end)

    test("6.02 mixed room: portal north + legacy east", function()
        -- Room has a portal reference for north AND a legacy exit for east
        local portal_n = make_portal_north({ _state = "open" })
        local reg = registry_mod.new()
        reg:register(portal_n.id, portal_n)
        -- Also register south portal so ctx is complete
        local portal_s = make_portal_south({ _state = "open" })
        reg:register(portal_s.id, portal_s)
        ...
        local room = {
            id = "mixed-room",
            name = "Mixed Room",
            description = "A room with both types of exits.",
            contents = { portal_n.id },
            exits = {
                north = { portal = "bedroom-hallway-door-north" },
                east = {
                    target = "east-room",
        ...
        local ctx = {
            registry = reg,
            current_room = room,
            rooms = { ["mixed-room"] = room, hallway = hallway, ["east-room"] = east_room },
            player = {
                location = "mixed-room",
                hands = { nil, nil },
                worn = {},
        ...
        -- Test portal exit (north)
        capture_output(function() handlers["go"](ctx, "north") end)
        -- This test may fail until portal resolution is implemented
        if ctx.player.location == "hallway" then
            eq("hallway", ctx.player.location, "portal north should work")
        else
            pending("6.02a portal north in mixed room",
                "portal exit resolution not implemented — player at: " .. ctx.player.location)
        end

        -- Reset to mixed-room for legacy test
        ctx.player.location = "mixed-room"
        ctx.current_room = room

        -- Test legacy exit (east)
        capture_output(function() handlers["go"](ctx, "east") end)
        eq("east-room", ctx.player.location,
            "legacy exit east should still work in mixed room")
    end)

    test("6.03 legacy locked exit blocks movement", function()
        local reg = registry_mod.new()
        local room = {
            id = "test-room",
            name = "Test Room",
        ...
        eq("test-room", ctx.player.location,
            "locked legacy exit should block movement")
        h.assert_truthy(out:lower():find("locked"),
            "should mention locked — got: " .. out)
    end)
else
    pending("6.01 legacy exit works", "verb handlers not loaded")
    pending("6.02 mixed room exits", "verb handlers not loaded")
    pending("6.03 legacy locked exit blocks", "verb handlers not loaded")
end
