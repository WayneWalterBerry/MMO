...
# WAVE 3 BURNDOWN PLAN — 2026-03-28
**Status:** Pre-Wave (Bug fixes before Phase 5 starts)  
**Scope:** 54 actionable issues triaged and categorized  
**Closed by Session:** 17 issues (Wave 1-2 fixes + #261 stress + #367 duplicate)
---
## Executive Summary
Phase 4 completed all 6 waves. We shipped 74+ objects, 7 rooms, 31+ verbs, and the crafting loop. Burndown sessions today closed 15 Wave 1-2 integration bugs during walk-away execution.
**This Wave:** 54 remaining open issues triaged into:
- **Wave 3 (Actionable Bugs):** 39 issues ready for immediate fix
- **Deferred (Phase 5 work):** 15 issues blocked on design, scope, or sequencing
---
## WAVE 3 ASSIGNMENT (39 bugs)
### Tier P0 — Critical (6 bugs)
These block gameplay loops or crash the parser.
| # | Title | Owner | Type | Notes |
|---|-------|-------|------|-------|
| #343 | Living creature passes alive check — can butcher living wolf | Flanders | Logic | Butchery LSM guard missing |
| #308 | Rest verb does not call cure_stress — stress never cured | Smithers | Parser/Verbs | cure_stress not wired to rest |
| #311 | cure_stress ignores duration requirement from stress.lua | Bart | Engine/FSM | Duration lookup missing |
| #312 | Territory markers never instantiated by wolf — territorial system non-functional | Flanders | Objects | Wolf object lacks marker spawn logic |
| #315 | Pack tactics untestable — only 1 wolf in game (max_population = 1) | Bart | Engine/Config | Registry capacity blocker |
| #323 | Wolf scent vanishes instantly when wolf leaves room — no lingering territorial scent | Bart | Engine/Effects | Scent effect lacks persistence |
### Tier P1 — High (15 bugs)
Core gameplay issues, wrong outputs, or unsafe state transitions.
| # | Title | Owner | Type | Notes |
|---|-------|-------|------|-------|
| #319 | Wolf never retreats in player combat — max_health lookup bug | Bart | Engine/Combat | Health threshold lookup broken |
| #324 | Wolf never attacks player — player not in prey list | Flanders | Objects | Wolf AI prey list incomplete |
| #329 | use poultice bypasses no-injury guard, wastes consumable | Smithers | Parser/Verbs | Guard condition missing |
| #330 | No standalone flee/run verb outside combat stance system | Smithers | Parser/Verbs | Exit verb not wired |
| #331 | attack dead creature says 'don't see' instead of 'already dead' | Smithers | Parser/Verbs | Error message wrong state check |
| #338 | Garbled spider bite narration — broken interpolation, incomplete sentences | Smithers | UI/Output | Narrative string malformed |
| #341 | Dead rat corpse spoilage FSM advances through all states in 3 commands | Bart | Engine/FSM | State transition guard missing |
| #344 | 'attack creature' skips disambiguation when multiple creatures present | Smithers | Parser | Multi-target context lost |
| #345 | Death messages use lowercase article: 'a brown rat is dead!' | Smithers | UI/Output | Capitalization bug |
| #346 | Ambient smell/listen room scan excludes rat | Bart | Engine/Senses | Scan filter wrong |
| #370 | Death message should use 'the rat' not 'a brown rat' after first mention | Smithers | UI/Output | Anaphora tracking missing |
| #309 | Door disambiguation shows identical names for two different doors in Storage Cellar | Moe | Objects/Rooms | Door naming conflict |
| #303 | Craft system doesn't find ingredients on room floor | Bart | Engine/Search | Ingredient discovery traversal bug |
| #313 | 'light candle' while holding candle holder targets holder instead of nested candle | Smithers | Parser | Scope resolution bug |
| #337 | Spider body zones reference human anatomy (knee, thigh, shin) in combat | Flanders | Objects | Body tree wrong species |
### Tier P2 — Medium (12 bugs)
Edge cases, unclear UX, or non-blocking errors.
| # | Title | Owner | Type | Notes |
|---|-------|-------|------|-------|
| #305 | 'use bandage' not recognized despite being listed as application verb | Smithers | Parser/Verbs | Verb alias missing |
| #306 | 'stomp spider' not recognized — natural combat phrase | Smithers | Parser/Verbs | Combat alias missing |
| #307 | 'craft' with no noun gives misleading error message | Smithers | Parser/Verbs | Error message UX |
| #310 | Parser ignores 'with' tool modifier for unlock/lock verbs | Smithers | Parser | Preposition handling |
| #317 | No player feedback for sub-threshold stress accumulation | Smithers | UI/Output | Silent state change |
| #318 | 'exits' command not recognized | Smithers | Parser/Verbs | Verb missing |
| #320 | 'insert key into lock' produces nonsensical 'You can't close a small brass key' | Smithers | Parser/Verbs | Error message mangled |
| #321 | 'use key on padlock' not recognized | Smithers | Parser/Verbs | Verb alias missing |
| #322 | 'unbar door' says 'You aren't holding that' — unclear unbar mechanic | Smithers | Parser/Verbs | Error handling |
| #335 | parser fuzzy-matches 'meat' to 'rug' for eat/cook commands | Bart | Parser/Fuzzy | Fuzzy rank too lenient |
| #369 | Combat targets 'knee' on spider despite body_tree not having knee | Flanders | Objects | Body tree validation missing |
| #368 | Rename 'goto' debug verb to 'teleport' | Smithers | Parser/Verbs | Debug verb cosmetic |
### Tier P3 — Low (6 bugs)
Ghost objects, parser edge cases, or design clarification needed.
| # | Title | Owner | Type | Notes |
|---|-------|-------|------|-------|
| #294 | Can't hold two items of same type — 'You already have that' blocks silk-rope crafting | Bart | Engine/Inventory | Inventory de-dup logic too aggressive |
| #296 | Spider web is a ghost object — visible in description but not interactable | Flanders | Objects | Object not instantiated |
| #299 | Disambiguation prompt for identical items gives no way to differentiate | Bart | Engine/Parser | Prompt UX missing context |
| #314 | cure_stress and is_safe_room use different safety check methods | Bart | Engine/Consistency | Inconsistent API |
| #342 | 'look the rat' fails — article not stripped by look verb | Smithers | Parser | Input normalization bug |
| #265 | Wearing clothes requires empty hands + no pocket storage on worn items | Flanders | Objects/Feature | Design clarification needed |
---
## DEFERRED (15 issues) — Phase 5+
### Portal Refactoring (6 issues) — Phase 5, Lisa
Portal Phase 2 TDD refactoring. Pre-Wave parallel task, but deferred from Wave 3 to Phase 5 formal work.
| # | Title |
|---|-------|
| #203 | portal: TDD + refactor deep cellar-hallway stairway |
| #204 | portal: TDD + refactor deep cellar-crypt archway |
| #205 | portal: TDD + refactor hallway-level2 staircase |
| #206 | portal: TDD + refactor hallway-west door |
| #207 | portal: TDD + refactor hallway-east door |
| #208 | portal: TDD + refactor courtyard-kitchen door |
### Puzzle 017 (Deep-Cellar Chain Mechanism) (5 issues) — Phase 5
Blocked on Level 1 completion. Puzzle 017 requires new objects and room updates.
| # | Title |
|---|-------|
| #126 | Room: Deep-cellar chain puzzle undefined + incense burner FSM missing |
| #254 | Puzzle 017: Update deep-cellar.lua room with chain mechanism + alcove |
| #255 | Puzzle 017: Create incense-stick.lua object |
| #256 | Puzzle 017: Create altar-candle.lua object |
| #257 | Puzzle 017: Create stone-alcove.lua object |
| #258 | Puzzle 017: Update incense-burner for incense-stick compatibility |
### Features & Design (4 issues) — Phase 5+
Feature requests and design clarifications blocked on Wayne's Phase 5 decisions.
| # | Title | Notes |
|---|-------|-------|
| #262 | Feature: Werewolf Disease (Lycanthropy) as an Injury type | Blocked on Q1 (werewolf design) |
| #263 | Feature: Rabies as an Injury/Disease type | Blocked on Q1 (disease model) |
| #250 | meta-lint: GUID-02 — 21 orphan objects not referenced by any room | Phase 2 inventory cleanup |
---
## Wave 3 Execution Model
### Assignments
- **Bart** (Engine/Parser/FSM): 12 bugs (P0, P1, P2)
- **Smithers** (Parser/Verbs/UI): 16 bugs (P0, P1, P2, P3)
- **Flanders** (Objects/Creatures): 8 bugs (P0, P1, P2, P3)
- **Moe** (Rooms): 1 bug (P1)
### Dependencies
- P0 bugs (6) must complete before Phase 5 starts (they block ecosystem testing)
- P1 bugs (15) should complete Wave 3 (non-blocking but core gameplay)
- P2 bugs (12) can roll into Phase 5 if time-boxed
- P3 bugs (6) are nice-to-have polish
### Success Criteria
- All P0 bugs fixed (stress system, territorial wolves, butchery guards)
- All P1 bugs fixed (combat loops, creature AI, dark sense perception)
- P2+ bugs triaged to team charters (self-directed in their domain)
- Remaining 15 deferred issues marked as Phase 5 (Portal + Puzzle + Features)
---
## Notes
### Sessions Closed Today
- **Wave 1 fixes:** 7 bugs (Moe, Bart, Smithers, Flanders)
- **Wave 2 fixes:** 7 bugs (Smithers, Bart)
- **Plus:** #261 (stress injury), #367 (duplicate)
- **Total closed:** 17 issues (31% reduction)
### Duplicates Found
- **#367** ← **#366** (Someone→You pronoun; #366 already fixed)
- **#336, #350** already closed (Wave 1 fixes for verb aliases)
- **#351, #356** already closed (earlier in session)
### Known Risks
1. **Parser flakiness:** Fuzzy matching may rank incorrectly (#335). Needs threshold review.
2. **FSM guards:** Multiple state-transition guards missing (#311, #341). Systematic audit recommended.
3. **Inventory de-dup:** #294 blocks crafting; needs careful fix to preserve duplicate detection.
4. **Body tree validation:** Spider anatomy wrong (#337, #369); Flanders should audit all creature bodies.
---
## Next Steps (for Phase 5 Pre-Wave)
1. ✅ Triage complete (this document)
2. → Assign Wave 3 bugs to team charters
3. → Spawn agents for P0 bugs (stress, territorial wolves, butchery)
4. → Run integration smoke test after each team completes
5. → Defer Portal/Puzzle issues to Phase 5 formal backlog
---
**Plan by:** Chalmers (Project Manager)  
**Date:** 2026-03-28  
**Next Review:** After Wave 3 execution complete (pre-Phase 5 start)