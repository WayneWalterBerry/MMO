# Comic Book Guy — History Archive

**Archived:** 2026-03-27
**Cutoff date:** 2026-07-14 (entries older than this date)
**Source:** `.squad/agents/comic-book-guy/history.md`
...
### Session Update: Prime Directive Tiers 1–5 Design Spec (2026-03-25)
**Status:** ✅ DESIGN COMPLETE
**Issue:** #106

**Deliverable:** `docs/design/prime-directive-tiers.md` — ~49 KB comprehensive parser tier design specification.

**What was done:**
- Designed all 5 parser tiers from the player experience perspective (Tier 0 already implemented)
- Tier 1 (Question Transforms): Catalogued missing question patterns — "why" causal questions, capability/comparative/counting questions, rhetorical question handling
- Tier 2 (Error Messages): Defined 5 error categories (unknown verb, unknown noun, impossible action, missing prerequisite, ambiguous target) with progressive hint system and narrator voice guide
- Tier 3 (Idiom Library): Organized expansion by action category (movement, examination, manipulation, sensory, inventory); set diminishing returns threshold at 80–120 entries
- Tier 4 (Context Window): Designed command history, disambiguation memory, salience scoring, implicit object inference, recency decay
- Tier 5 (Fuzzy Noun Resolution): Proposed typo threshold relaxation, color/state-based matching, confidence tiers with narration pattern, "Did you mean?" confirmation flow
- Established priority order: Tier 2 > Tier 5 > Tier 4 > Tier 1 > Tier 3
- Included classic IF references (Infocom, Magnetic Scrolls, Level 9, modern Inform 7)
- Proposed "OOPS" command (from Level 9) — trivial to implement, huge UX win
- Built Infocom capability comparison table showing where we're ahead and behind

**Key Design Decisions:**
- **Error messages are content, not failures** — narrator voice should be dry, witty, in-world
- **Progressive hints escalate:** 1st fail = gentle redirect, 2nd = verb list, 3rd = personality
- **Fuzzy confidence tiers:** score ≥5 = execute, 3–4 = execute with narration, 2 = confirm, ≤1 = fail
- **Salience scoring** resolves "the door" to the most contextually relevant door (recency + proximity + mention)
- **Idiom library capped at ~120 entries** — beyond that, invest in embedding matching instead
- **NEVER reveal hidden objects** in error messages — "You don't see that" is truthful; "There is no X" is a lie
- **Disambiguation memory** — game should remember the options it just asked about for 3 commands
