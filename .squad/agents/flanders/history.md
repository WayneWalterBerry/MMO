...
### Poison Bottle Upgrade + Bear Trap + Crushing Wound (2026-07-25)
- **Upgraded `src/meta/objects/poison-bottle.lua`** — Added consumable metadata (`is_consumable`, `consumable_type = "liquid"`, `poison_type = "nightshade"`, `poison_severity = "lethal"`), upgraded `effect = "poison"` (legacy string) to structured effect table `{ type = "inflict_injury", injury_type = "poisoned-nightshade", ... }` per Bart's effect processing pipeline architecture. Added `on_taste_effect` structured table to open state (weaker dose, damage=5). Added label as non-detachable readable part with `readable_text` containing the poison warning. Added `requires_free_hands = true` to cork removal transitions. Added `consume` alias to drink transition. Added GOAP prerequisites block. Added categories: "consumable", "liquid", "poison". Mutate on drink/pour now sets `is_consumable = false`.
- **Created `src/meta/objects/bear-trap.lua`** `{d7e3b1a4-5c92-4f08-b6d1-8a3e7c4f29b5}` — New touch-triggered injury object. Three FSM states: set (armed, dangerous), triggered (snapped, safe to handle), disarmed (neutralized, portable). Contact triggers on take/touch verbs when armed — both use structured effect tables inflicting `crushing-wound`. Set state has `on_feel_effect` structured table as well. Disarm transition uses `guard` function checking `context.player.has_skill("lockpicking")` + `requires_tool = "thin_tool"`. Self-transitions for safe take in triggered/disarmed states. Trap metadata: `is_trap`, `is_armed`, `is_dangerous`, `trap_type = "spring-jaw"`. Mutate fields flip armed/dangerous/sprung flags and manage category transitions (dangerous→evidence→trophy). Visible by default (no `hidden = true`).
- **Created `src/meta/injuries/crushing-wound.lua`** `{8b3f6a21-d4c7-4e59-a1b2-5c8d9e0f3a74}` — New injury template for bear trap. Hybrid damage: initial 15 + 2/tick bleeding from crushed tissue. FSM: active(12 turns)→worsened(10 turns)→critical(5 turns)→fatal (untreated) or active→treated(15 turns)→healed (bandaged). Treatment from worsened takes 20 turns; from critical takes 30 turns (via `_timer_delay` mutate). Restricts: grip+climb+fight (active), adds run (worsened), adds focus (critical). Healing interactions: bandage (active+worsened), healing-poultice (active+worsened+critical). Distinct from pure bleeding — crushing pain persists after bandaging, only bleeding component stops.
