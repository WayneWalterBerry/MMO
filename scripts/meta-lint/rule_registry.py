...
class RuleMeta:
    ...
    fix_safety: str         # "safe" or "unsafe" (only meaningful when fixable=True)
...
def _r(id: str, severity: str, category: str, description: str,
       fixable: bool = False, fix_safety: str = "unsafe") -> None:
    _RULES[id] = RuleMeta(
...
_r("S-01",     "error",   "structure", "File must return a table")
_r("S-02",     "error",   "structure", "Missing guid field")
_r("S-04",     "error",   "structure", "Missing id field")
_r("S-06",     "error",   "structure", "Missing name field")
_r("S-07",     "error",   "structure", "Missing or unknown template field")
_r("S-09",     "error",   "structure", "Keywords table missing or empty")
_r("S-10",     "error",   "structure", "Keywords must be strings")
_r("S-11",     "warning", "structure", "Description missing or wrong type")

# ── GUID ─────────────────────────────────────────────────────────────────────
_r("G-01",     "error",   "guid",      "Invalid GUID format")
...
_r("TD-06",    "error",   "template",  "Template keywords table exists")
_r("TD-07",    "error",   "template",  "Template description exists")
...
_r("TD-11",    "error",   "template",  "Physical template: size > 0")
_r("TD-12",    "error",   "template",  "Physical template: weight > 0")
_r("TD-13",    "error",   "template",  "Physical template: portable is boolean")
_r("TD-14",    "error",   "template",  "Physical template: material is string")
_r("TD-15",    "error",   "template",  "Physical template: container is boolean")
_r("TD-16",    "error",   "template",  "Physical template: capacity >= 0")
_r("TD-17",    "error",   "template",  "Physical template: contents table exists")
_r("TD-18",    "warning", "template",  "Template contents should be empty")
_r("TD-19",    "info",    "template",  "Template should declare location = nil")
_r("TD-20",    "warning", "template",  "Categories table of strings")
_r("TD-21",    "error",   "template",  "Container template: container = true")
_r("TD-22",    "error",   "template",  "Container template: capacity > 0")
_r("TD-23",    "warning", "template",  "Container template: weight_capacity should be > 0")
_r("TD-24",    "error",   "template",  "Room template: no physical properties")
_r("TD-25",    "error",   "template",  "Room template: contents table exists")
_r("TD-26",    "error",   "template",  "Room template: exits table exists")
_r("TD-27",    "warning", "template",  "Sheet material should be fabric-class")
...
_r("INJ-06",   "error",   "injury",    "Injury category field exists")
_r("INJ-07",   "warning", "injury",    "Injury category is known type")
_r("INJ-08",   "error",   "injury",    "Injury description exists")
_r("INJ-10",   "info",    "injury",    "No template field on injury")
_r("INJ-11",   "error",   "injury",    "Injury damage_type exists")
_r("INJ-12",   "error",   "injury",    "Injury damage_type is known")
_r("INJ-13",   "error",   "injury",    "Injury initial_state exists")
_r("INJ-14",   "error",   "injury",    "Injury initial_state references defined state")
_r("INJ-15",   "error",   "injury",    "on_inflict table exists")
_r("INJ-16",   "error",   "injury",    "on_inflict.initial_damage >= 0")
_r("INJ-17",   "error",   "injury",    "on_inflict.damage_per_tick >= 0")
_r("INJ-18",   "error",   "injury",    "on_inflict.message exists")
_r("INJ-19",   "warning", "injury",    "damage_type/damage_per_tick consistency")
_r("INJ-20",   "error",   "injury",    "States table exists")
_r("INJ-21",   "error",   "injury",    "At least 2 states")
_r("INJ-22",   "error",   "injury",    "Each state has name")
_r("INJ-23",   "error",   "injury",    "Each state has description")
_r("INJ-24",   "error",   "injury",    "Non-terminal state has on_feel")
_r("INJ-25",   "error",   "injury",    "Non-terminal state has damage_per_tick")
_r("INJ-26",   "error",   "injury",    "Named states (healed/fatal) should be terminal")
_r("INJ-27",   "error",   "injury",    "At least one terminal state")
...
_r("INJ-32",   "info",    "injury",    "Non-terminal state should have on_look")
_r("INJ-33",   "info",    "injury",    "Bleeding/infected states should have on_smell")
_r("INJ-34",   "error",   "injury",    "timed_events must be table")
_r("INJ-35",   "error",   "injury",    "Timed event has event field")
_r("INJ-36",   "error",   "injury",    "Timed event has positive delay")
_r("INJ-37",   "error",   "injury",    "Timed event has to_state")
_r("INJ-38",   "error",   "injury",    "Timed event to_state in states")
_r("INJ-39",   "warning", "injury",    "Timed event delay in range (360-10800)")
_r("INJ-40",   "warning", "injury",    "Multiple transition events")
_r("INJ-41",   "error",   "injury",    "restricts must be table")
_r("INJ-42",   "error",   "injury",    "restricts action = true")
_r("INJ-43",   "warning", "injury",    "Unknown restrict actions")
_r("INJ-44",   "error",   "injury",    "transitions entries must be tables")
_r("INJ-45",   "error",   "injury",    "Transition has from")
_r("INJ-46",   "error",   "injury",    "Transition has to")
_r("INJ-47",   "error",   "injury",    "Transition from state in states")
_r("INJ-48",   "error",   "injury",    "Transition to state in states")
_r("INJ-49",   "error",   "injury",    "Non-auto transition has verb")
_r("INJ-50",   "error",   "injury",    "Trigger must be auto if present")
_r("INJ-51",   "warning", "injury",    "Auto transition should have condition")
_r("INJ-52",   "error",   "injury",    "Transition has message")
_r("INJ-53",   "error",   "injury",    "Transition not from terminal state")
_r("INJ-54",   "warning", "injury",    "Duplicate from+verb pairs")
_r("INJ-55",   "error",   "injury",    "requires_item_cures is string")
_r("INJ-56",   "error",   "injury",    "mutate is table")
_r("INJ-57",   "warning", "injury",    "mutate.damage_per_tick >= 0")
_r("INJ-58",   "error",   "injury",    "healing_interactions table exists")
_r("INJ-59",   "error",   "injury",    "healing_interactions is table")
_r("INJ-60",   "error",   "injury",    "healing_interactions item has transitions_to")
_r("INJ-61",   "error",   "injury",    "transitions_to in states")
_r("INJ-62",   "error",   "injury",    "healing_interactions item has from_states")
_r("INJ-63",   "error",   "injury",    "from_states entries in states")
_r("INJ-64",   "warning", "injury",    "from_state not terminal")
_r("INJ-66",   "error",   "injury",    "causes_unconsciousness is boolean")
_r("INJ-67",   "error",   "injury",    "unconscious_duration table with positive numbers")
_r("INJ-68",   "error",   "injury",    "unconscious_duration requires causes_unconsciousness=true")
_r("INJ-69",   "info",    "injury",    "Unknown top-level fields")
...
_r("MD-08",    "error",   "material",  "flexibility in [0.0, 1.0]")
_r("MD-09",    "error",   "material",  "absorbency in [0.0, 1.0]")
_r("MD-10",    "error",   "material",  "opacity in [0.0, 1.0]")
_r("MD-11",    "error",   "material",  "flammability in [0.0, 1.0]")
_r("MD-12",    "error",   "material",  "conductivity in [0.0, 1.0]")
_r("MD-13",    "error",   "material",  "fragility in [0.0, 1.0]")
_r("MD-14",    "error",   "material",  "value > 0")
_r("MD-15",    "error",   "material",  "melting_point positive or nil")
_r("MD-16",    "error",   "material",  "ignition_point positive or nil")
...
_r("MD-19",    "warning", "material",  "Melting/ignition point conflict", fixable=False)
...
_r("MD-22",    "error",   "material",  "rust_susceptibility in [0.0, 1.0]")
...
_r("LV-02",    "error",   "level",     "template = level")
_r("LV-03",    "error",   "level",     "number is positive integer")
_r("LV-04",    "error",   "level",     "name exists")
_r("LV-05",    "error",   "level",     "rooms table non-empty with string entries")
...
_r("LV-11",    "error",   "level",     "intro is table")
_r("LV-12",    "error",   "level",     "intro.title non-empty string")
_r("LV-13",    "error",   "level",     "intro.narrative table of strings")
_r("LV-14",    "warning", "level",     "intro.narrative non-empty")
_r("LV-15",    "warning", "level",     "intro.help non-empty string")
_r("LV-16",    "info",    "level",     "intro.subtitle should be string")
_r("LV-17",    "error",   "level",     "completion table of tables")
_r("LV-18",    "error",   "level",     "Each completion entry has type")
_r("LV-19",    "error",   "level",     "type=reach_room requires room")
_r("LV-20",    "error",   "level",     "completion room in rooms list")
_r("LV-21",    "warning", "level",     "completion entry should have message")
_r("LV-22",    "warning", "level",     "completion from references valid room")
_r("LV-23",    "warning", "level",     "boundaries should be table")
_r("LV-24",    "error",   "level",     "boundaries.entry table of strings, non-empty")
_r("LV-25",    "error",   "level",     "boundaries.entry rooms in level rooms")
_r("LV-26",    "warning", "level",     "start_room in boundaries.entry")
_r("LV-27",    "warning", "level",     "boundaries.exit table of tables")
_r("LV-28",    "error",   "level",     "boundary exit has room")
_r("LV-29",    "error",   "level",     "boundary exit has exit_direction")
_r("LV-30",    "error",   "level",     "boundary exit has positive target_level")
_r("LV-31",    "error",   "level",     "boundary exit room in rooms list")
_r("LV-33",    "warning", "level",     "target_level > current level")
_r("LV-34",    "error",   "level",     "restricted_objects table of strings")
_r("LV-35",    "warning", "level",     "restricted objects reference existing objects")
_r("LV-36",    "error",   "level",     "description is non-empty string")
_r("LV-37",    "error",   "level",     "rooms entries are unique")
...
_r("SN-02",    "error",   "sensory",   "on_feel is string or function")

# ── FSM ──────────────────────────────────────────────────────────────────────
_r("FSM-01",   "error",   "fsm",       "initial_state required if states defined")
_r("FSM-04",   "error",   "fsm",       "initial_state defined in states")

# ── Transition ───────────────────────────────────────────────────────────────
_r("TR-01",    "error",   "transition", "Transition from state in states")
_r("TR-02",    "error",   "transition", "Transition to state in states")
