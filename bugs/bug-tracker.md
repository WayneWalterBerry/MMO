...
|--------|---------|----------|------------|----------|-----------------|
| BUG-069 | `sleep` → dawn error message is wrong | pass-022 | yes | smithers | ✅ |
| BUG-070 | Excessive blank lines in output | pass-023 | yes | smithers | ❌ needs test |
| BUG-071 | Rapid `look around` command spam can hang game | pass-023 | yes | smithers (cannot-reproduce; pipeline is iterative, handler has no recursion; 100x stress test passes) | ✅ |
...
| BUG-104b | `please could you have a look around` — politeness + idiom combo | pass-032 | yes | smithers (pipeline order correct: strip_filler before expand_idioms) | ✅ |
| BUG-105 | `what do I do?` hangs | pass-031 | yes | smithers (belt-and-suspenders) | ✅ |
| BUG-105b | Bare `examine` (no noun) says "can't find that" instead of asking what | pass-032 | yes | smithers (added examine to no_noun_verbs) | ✅ |
| BUG-106 | `what now?` hangs | pass-031 | yes | smithers (belt-and-suspenders) | ✅ |
| BUG-106b | `blow out the candle` (unlit) says "can't extinguish" — implies candles can't be blown out | pass-032 | yes | smithers (check full transitions for extinguish capability) | ✅ |
